...
import { RecommendationBannerComponent } from './components/recommendation-banner/recommendation-banner';
...
@Component({
  ...
  template: `
    ...
    <ion-content class="dashboard-content">
      ...
      <app-stats-card
        [totalJobs]="jobService.stats().total"
        [topScore]="jobService.stats().highMatch"
        [avgScore]="jobService.stats().avgScore"
        [remoteCount]="jobService.stats().remoteCount"
      ></app-stats-card>

      <!-- Quick Filters Component -->
      <app-quick-filters
        [activeFilter]="activeQuickFilter"
        (filterSelect)="applyQuickFilterById($event)"
      ></app-quick-filters>
      ...
      @if (!jobService.isLoading()) {
        @if (jobService.filteredJobs().length === 0) {
          ...
          @if (jobService.filteredJobs()[0] && profileService.hasProfile()) {
            <app-recommendation-banner
              [topJob]="jobService.filteredJobs()[0]"
              (viewJob)="onApply($event)"
...
export class DashboardPage implements OnInit, OnDestroy {
  ...
  applyQuickFilterById(filterId: string): void {
    const isActive = this.activeQuickFilter === filterId;

    switch (filterId) {
      case 'remote':
        this.jobService.setRemoteOnly(!isActive);
        break;
      case 'global':
        this.jobService.setLocation(!isActive ? 'Global' : '');
        break;
      case 'trending':
        this.jobService.setTimeFilter(!isActive ? '24h' : '7d');
        break;
      case 'junior':
        this.jobService.setSearchQuery(!isActive ? 'junior' : '');
        break;
      case 'senior':
        this.jobService.setSearchQuery(!isActive ? 'senior' : '');
        break;
      case 'startup':
        this.jobService.setSearchQuery(!isActive ? 'startup' : '');
        break;
      case 'fulltime':
        this.jobService.setSearchQuery(!isActive ? 'full time' : '');
        break;
    }

    // Toggle active state
    this.activeQuickFilter = isActive ? '' : filterId;
  }
